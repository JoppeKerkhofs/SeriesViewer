...
export async function initializeShow(name: string) {
	...
	let location =
		showData.Title.toLowerCase()
			.replace(/\s/g, "")
			.replace(/[^\w\s]/gi, "") + "/posters";
	...
	for (let i = 1; i <= parseInt(showData.totalSeasons); i++) {
		try {
			...
			for (let j = 1; j <= seasonData.Episodes.length; j++) {
				try {
					const episodeData: EpisodeResponse = await new Promise(
						(resolve) => {
							ipcRenderer.send("get-episode", name, i, j);
							ipcRenderer.once(
								"get-episode",
								(event, data: EpisodeResponse) => {
									resolve(data);
								}
							);
						}
					...
					let location =
						showData.Title.toLowerCase()
							.replace(/\s/g, "")
							.replace(/[^\w\s]/gi, "") + "/episodes";
					// get the poster of the episode
					const episodePoster: string = await new Promise(
						(resolve) => {
							ipcRenderer.send(
								"get-online-image",
								episodeData.Poster,
					...
					season.episodes.push(
						new Episode(
							show.id,
							i,
							parseInt(episodeData.Episode),
							episodeData.Title,
							parseInt(episodeData.Runtime),
							episodePoster
						)
					);
